...
const HashTable = struct {
    ...
    const Waiter = union(enum) {
        ...
        fn park(self: *Waiter, timeout: ?Time) bool {
            switch (self.*) {
                .thread => |*thread| {
    ...
    fn init(allocator: Allocator, num_waiters: usize) *HashTable {
...
pub fn init(allocator: std.mem.Allocator) Self {
    return Self{ .arena = .init(allocator) };
}
...
fn getHashTable(self: *Self) *HashTable {
    ...
    const new_table = HashTable.init(self.arena.allocator(), HashTable.load_factor);
    if (self.hash_table.cmpxchgStrong(null, new_table, .acq_rel, .acquire)) |old| return old.?;
    return new_table;
}
...
fn growHashTable(self: *Self, num_waiters: usize) void {
    ...
    const new_table = HashTable.init(self.arena.allocator(), num_waiters);
...
pub fn parkMultiple(
    ...
    const max_num_keys = 128;
    if (keys.len == 0 or keys.len > max_num_keys)
        return ParkMultipleResult{ .type = .keys_invalid };
    for (keys, 0..) |k, i| if (std.mem.count(*const anyopaque, keys[0..i], &k) != 0)
        return ParkMultipleResult{ .type = .keys_invalid };

    var multi_queue = createMultiQueueEntry(token);
    var entries_array: [max_num_keys]HashTable.Entry = undefined;
